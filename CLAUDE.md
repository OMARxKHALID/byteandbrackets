...
- `app/not-found.js` is the custom 404 — `robots: noindex/nofollow`, deliberately absent from the sitemap. Don't add it to `staticRoutes`.

## Design tokens

The site is a dark theme built on **Tailwind's default color palette** (no custom color tokens — `@theme` in `app/globals.css` only declares the three font variables). The three roles map to default tokens: **`lime-100`** `#ecfcca` (text/cream — the light role), **`lime-400`** `#9de500` (accent), **`neutral-950`** `#0a0a0a` (surfaces/panels). The page background is pure black (`body`); panels/cards use `bg-neutral-950` for subtle elevation. Express every in-between tone as a Tailwind **opacity modifier** of these (`text-lime-100/60` for secondary text, `border-lime-100/10`–`/15` for dividers, `bg-lime-100/[0.05]` for hover lifts). Stick to default Tailwind theme variables — do not introduce custom color vars or raw hexes. Fonts loaded via `next/font/google` in layout and exposed as `--font-display` (Saira Stencil One — a single-weight 400 stencil display face; do not pair `font-bold`/`font-extrabold` with `font-display`, it only triggers faux-bold), `--font-body` (Hanken Grotesk), `--font-mono` (JetBrains Mono); use the `font-display` / `font-mono` utilities.
...
- Icons come from `react-icons` (the only icon source; currently used in `marquee-section.jsx`). Don't add inline SVGs or another icon package.
- The lone custom hook is `hooks/use-scroll-spy.js` — an `IntersectionObserver` wrapper that drives the active nav link; reuse it for scroll-position state rather than re-implementing observers.
