...
Byte & Brackets — a single-page marketing site for a software studio, plus a `/projects` index and `/work/[slug]` case-study pages. Next.js 16 App Router, React 19, Tailwind CSS v4, `motion` (Framer Motion successor) for animation. Content is fictional placeholder data.
...
- `app/sitemap.js` already maps every `PROJECTS` slug to a `/work/*` URL automatically; only its hardcoded `staticRoutes` array (`""`, `/projects`) needs a manual edit when adding a new static page. `app/robots.js` and `public/llms.txt` also exist — keep `llms.txt` in sync with copy changes.
...
## Conventions (from rules.md + global rules)
